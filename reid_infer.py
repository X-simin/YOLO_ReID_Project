...
# 【终极平衡参数】
YOLO_CONF_THRESHOLD = 0.68
SIM_THRESHOLD = 0.68
MAX_MISS_FRAMES = 15        # 目标存活时间长，保证ID稳定
MIN_BOX_RATIO = 1.25
MIN_BOX_SIZE = 32
...
def match_persons(detections):
    ...
    if not tracked_persons:
        ...
        return tracked_persons, set(range(len(tracked_persons)))
    ...
    return tracked_persons, matched_tracker_indices
...
def process_frame(frame):
    ...
    matched_persons, matched_tracker_indices = match_persons(detections)
    ...
    # 【修复】只绘制在当前帧匹配成功的目标，避免索引越界
    for idx in matched_tracker_indices:
        if idx < len(tracked_persons):
            p = tracked_persons[idx]
            x1, y1, x2, y2 = p.bbox
